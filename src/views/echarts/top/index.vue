<template>
  <div class="home">
    <el-row :gutter="20">
      <el-col :span="6"
        ><Card :title="'累计销售额'" :number="reportData.salesToday">
          <template #content>
            <div class="info">
              <div class="footer">
                日同比<span class="increase"
                  >{{ reportData.salesGrowLastDay }}%</span
                >
              </div>
              <div class="footer">
                月同比<span class="increase"
                  >{{ reportData.saleSGrowLastMonth }}%</span
                >
              </div>
            </div>
          </template>
          <template #footer>
            <div class="footer">
              昨日销售额 <span>￥{{ reportData.salesLastDay }}</span>
            </div>
          </template>
        </Card></el-col
      >
      <el-col :span="6"
        ><Card :title="'累计订单额'" :number="reportData.orderToday">
          <template #content>
            <v-chart :option="optionOfOrder" />
          </template>
          <template #footer>
            <div class="footer">
              昨日销售额 <span>￥{{ reportData.orderLastDay }}</span>
            </div>
          </template>
        </Card></el-col
      >
      <el-col :span="6"
        ><Card :title="'今日用户交易数'" :number="reportData.userToday">
          <template #content>
            <v-chart :option="optionOfUser" />
          </template>
          <template #footer>
            <div class="footer">
              退货率 <span>{{ reportData.returnRate }}% </span>
            </div>
          </template>
        </Card></el-col
      >
      <el-col :span="6"
        ><Card :title="'累计用户数'" :number="reportData.totalUser">
          <template #content>
            <v-chart :option="optionOfTotalUser" />
          </template>
          <template #footer>
            <div class="footer user-footer">
              <div>
                同日比
                <span class="increase">{{ reportData.userGrowLastDay }}%</span>
              </div>
              <div>
                月同比
                <span class="increase"
                  >{{ reportData.userGrowLastMonth }}%</span
                >
              </div>
            </div>
          </template>
